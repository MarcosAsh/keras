...
class TestSpectrogram(testing.TestCase):
    ...
    def test_spectrogram_channels_broadcasting(self):
        ...
        self.assertAllClose(
            y_last, np.concatenate(y_singles, axis=-1), atol=1e-5, rtol=1e-5
        )
        self.assertAllClose(
            y_expanded, np.stack(y_singles, axis=-1), atol=1e-5, rtol=1e-5
        )
    ...
    def test_spectrogram_channels_first(self):
        ...
        self.assertAllClose(
            np.transpose(y_first, [0, 2, 1]), y_last, atol=1e-5, rtol=1e-5
        )
        self.assertAllClose(
            y_expanded, np.stack(y_singles, axis=1), atol=1e-5, rtol=1e-5
        )
